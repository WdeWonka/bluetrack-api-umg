...
def api_search_users(
    ...
    try:
        ...
        results = search_users(db, query=q, role=OPERATOR, skip=skip, limit=per_page)
...
@router.post(
    "/users/import",
    summary="Import operators users from Excel file",
    response_description="Operators imported successfully"
)
async def api_import_operators(
    file: UploadFile = File(..., description="Excel file (.xlsx) with operator data"),
    db: Session = Depends(get_db)
):
    """
    Import multiple operators from an Excel file.
    ...
    try:
        ...
        if not validation_errors and not db_errors:
            ...
            return HttpResponse.custom(
                message=f"Se crearon {len(created_users)} operadores exitosamente",
                response={
            ...
            return HttpResponse.custom(
                message="El archivo contiene errores de validación. No se creó ningún operador.",
                response={
            ...
            return HttpResponse.custom(
                message=f"Se crearon {len(created_users)} operadores. {len(db_errors)} operadores no se pudieron crear.",
                response={
                    "created_count": len(created_users),
...
def api_export_users(db: Session = Depends(get_db)):
    """
    Export all operator users to an Excel file.
...
@router.get(
    "/users/export/pdf",
    summary="Export all operators to PDF",
    response_description="PDF file with all operators"
)
def api_export_operators_pdf(db: Session = Depends(get_db)):
    ...
    **Returns:**
    ...
    try:
        ...
        return HttpResponse.internal_server_error(
            error="Ocurrió un error al exportar los operadores a PDF"
        )
...
def api_list_users(
    ...
    try:
        ...
        users = list_users(db, role=OPERATOR, skip=skip, limit=per_page)
