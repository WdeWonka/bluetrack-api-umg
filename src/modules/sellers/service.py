...
import logging
from src.models.user import User
from src.common.types.userType import UserCreate
...
def create_user(db: Session, user_data: UserCreate, role: str = SELLER) -> User:
    """
...
def list_users(db: Session, role: str = SELLER, skip: int = 0, limit: int = 10) -> list[User]:
    """
...
def count_users(db: Session, role: str = SELLER) -> int:
...
def search_users(db: Session, query: str, role: str = SELLER, skip: int = 0, limit: int = 5) -> list[User]:
    """
...
def count_search_results(db: Session, query: str, role: str = SELLER) -> int:
